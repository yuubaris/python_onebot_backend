...
def effective_stats(user, owned):
    ...
    if user is not None:
        try:
            from . import consumable as _consumable
    ...
    if user.dungeon_ore_eligible and user.dungeon_ore_last:
        ...
        if cycles > 0:
            ...
            for _ in range(cycles):
                # 每次采矿先掷数量（v2.11.36）：基础随层数增加 + 概率加成，必不掉空
                # 基础颗数 b = 1 + 每满 300 层 +1（封顶 4）；60% 得 b 颗 / 30% 得 b+1 颗 / 10% 得 b+2 颗
                # 例：100 层 1/2/3 · 300 层 2/3/4 · 600 层 3/4/5 · 900 层及以上 4/5/6
