# -*- coding: utf-8 -*-
"""祈愿模块：3 档祈愿（5 铜 / 5 银 / 5 金），各档中奖率与卡池稀有度不同。

- 档1（5 铜）：小奖为主——高概率小额铜币返还，实物多为低级装备/草药·矿石/低阶消耗品；
- 档2（5 银）：中奖率提升、稀有度上移——装备 tier2、稀有~传说材料/矿石、中阶消耗品；
- 档3（5 金）：高投入——大额金钱、tier4~5 装备、传说~神话材料/矿石、高阶消耗品、稀有装备大奖。
- 期望设计（v2.11.72/73）：三档均为**负期望**（EV/成本 ≈ 0.87 / 0.65 / 0.45），档位越高亏得越多，
  长期祈愿整体趋向亏损（铜币回收口径；材料/消耗品无定价未计入，实际略高于该值）。
  **高级货保留且高频**：档3 每抽 40% 出 tier4~5 装备（其中 20% 稀有大奖）、16% 出传说~神话材料
  （神话级 3 种：万瓜圣辉/万瓜圣契/万宝源晶）。

奖励发放：金钱 → user.copper；装备 → UserItem；材料（含草药/特殊材料）→ material.grant_materials，
矿石 → ore.grant_ores；消耗品 → consumable.grant_consumables。调用方负责 commit。
"""
...
def _award_mark(kind, tier_no, it=None, rarity=None, jackpot=False):
    """祈愿奖等（v2.11.78）：按奖品本身稀有度/价值分等，跨档一致。
...
def rule_text():
    """祈愿规则说明（三档成本 + 概率/稀有度概览）。"""
    lines = ["🎰 祈愿（/祈愿 1|2|3 或 /祈愿 铜|银|金）",
             "奖等（按奖品稀有度/价值，跨档一致）：🥉铜奖·低级 | 🥈银奖·中级 | 🥇金奖·高级(稀有大奖/传说材料) | 👑特等奖·神话材料"]
    for no in (1, 2, 3):
        t = TIERS[no]
        if no == 1:
            desc = "38% 谢谢惠顾 · 45% 小额铜币(1~5) · 6% 低级装备(tier0) · 6% 草药/材料/矿石 · 5% 低阶药水道具（期望≈0.87，长期略亏）"
        elif no == 2:
            desc = "30% 谢谢惠顾 · 38% 金钱(80~650) · 18% 装备(tier2) · 8% 稀有~传说材料/矿石 · 6% 中阶药水道具（期望≈0.65，亏损）"
        else:
            desc = "6% 谢谢惠顾 · 36% 金钱(1万~7万) · 40% 装备(tier4~5,20% 稀有掉落) · 16% 传说~神话材料/矿石 · 2% 高阶药水道具（期望≈0.45，仍亏；高级货高频）"
