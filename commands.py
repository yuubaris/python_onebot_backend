...
def cmd_help(user, group_id, args, at_qqs=None):
    return ("可用命令（消息以 / 开头）：\n"
            ...
            "/转职 战士|魔法师 - 选择职业（切换职业）\n"
            "/晋升 - 按地下城进度+货币提升阶级\n"
            "/地下城 进入/状态/排名/退出 - 地下城冒险（最高 3600 层；进入自动穿戴最优 4 件、推进只算穿戴中装备；命名守关 Boss 需战力判定，失败重置进度收益照常；状态含生效中药水/道具；排名看本群战力 TOP10 与专属拥有；套装：4件稀有全属性×1.1，命名Boss专属×1.2/件累乘（三大Boss×1.5/件、3600×2，稀有1.1不叠加））\n"
