...
- 装备带 tier(0~5)；玩家当前阶级 tier 决定能购买/装备到多好的装备，
  装备毕业在 T5=800（武具店顶配 + 铁匠铺全锻造均在此开放）。
...
# 装备 tier 上限。装备档位只到 5（武具店顶配 + 铁匠铺全锻造，均归 T5=800 毕业）；
# 6 灭世 / 7 至尊 为「纯称号」阶（无新装备/新解锁，仅荣誉）。
MAX_TIER = 7
...
# 末轮（按用户口径）：T0~T4 不变；T5=800（武具店顶配 + 铁匠铺全锻造在此开放）、
# T6=1600、T7=3600。T6 灭世 / T7 至尊 为纯称号荣誉阶（无新装备/解锁）。
TIER_LAYER = {
    0: 0,
    1: 30,
    2: 80,
...
TIER_PRICE_CAP = {
    ...
    3: 9000,
    4: 26000,
    5: 45000,      # 武具店顶配（T5=800 全解锁）
    6: 10 ** 9,    # 兜底档（T6/T7 纯称号阶不开放新装备档，此值仅供价格兜底）
}
